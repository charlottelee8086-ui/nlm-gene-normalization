...
OUT = Path("family_reranker_predictions_v3_hybrid.tsv")
...
def lexical_bonus(ex):
    ...
    for term in terms:
        ...
        # strongest: mention exactly equals candidate symbol/alias
        if mention == t:
            bonus += 1.0

        # strong: candidate symbol/alias appears in local context
        if t in context:
            bonus += 0.6

        # weak: candidate symbol/alias partially overlaps mention
...
with open(OUT, "w", encoding="utf-8") as out:
    out.write(
        "pmid\tmention\tpred_gid\tpred_symbol\tgold_gene_ids\t"
        "neural_score\tlexical_bonus\tfinal_score\tcorrect\toracle\n"
    )
    ...
    for key, examples in groups.items():
        ...
        best_final, best_neural, best_lex, best_gid, best_symbol = scores[0]
