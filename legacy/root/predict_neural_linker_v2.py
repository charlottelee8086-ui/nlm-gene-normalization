...
OUT = "pubmedbert_neural_linked_v2.PubTator"
...
# Build candidate KB from train set
    ...
    for ent in doc["entities"]:
        ...
        for norm in ent["normalized"]:
            if norm.get("db_name") == "NCBIGene":
                ...
                mention2ids[norm_key(mention)][gid] += 1
...
print("candidate mention keys:", len(mention2ids))
print("candidate gene ids:", len(gid2bestname))
...
with open(NER_PRED, encoding="utf-8") as f, open(OUT, "w", encoding="utf-8") as out:
    for line in f:
        ...
        candidates = list(mention2ids[mention.lower()].keys())

        if not candidates:
            candidates = list(mention2ids[norm_key(mention)].keys())
