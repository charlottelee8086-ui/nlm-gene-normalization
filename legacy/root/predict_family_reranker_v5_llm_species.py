...
OUT = Path("family_reranker_predictions_v5_llm_species.tsv")

SPECIES_BONUS = 0.8
LLM_SPECIES_BONUS = 1.0
...
def load_llm_species():
    lookup = {}

    with open("llm_species_lookup.tsv", encoding="utf-8") as f:
        next(f)

        for line in f:
            pmid, mention, taxid = line.rstrip("\n").split("\t")

            lookup[(pmid, mention)] = taxid
...
def llm_species_bonus(
    pmid,
    mention,
    candidate_taxid,
):
    llm_taxid = LLM_SPECIES.get(
        (pmid, mention)
    )
    ...
    if llm_taxid == "unclear":
        return 0.0

    if str(candidate_taxid) == str(llm_taxid):
        return LLM_SPECIES_BONUS
...
with open(
    ...
    out.write(
        ...
        "neural_score\tlexical_bonus\tspecies_bonus\tllm_species_bonus\t"
        "final_score\tcorrect\toracle\n"
    )
    ...
    for key, examples in groups.items():
        ...
        focus_taxid = pmid2taxid.get(
            pmid,
            ""
        )

        gold = set(
            examples[0]["gold_gene_ids"]
        )
